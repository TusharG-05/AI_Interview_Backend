...
def upgrade() -> None:
    ...
    # Use IF EXISTS guards — these tables/indexes may already be gone in some envs
    op.execute('DROP INDEX IF EXISTS ix_agentconversationturn_session_id')
    op.execute('DROP TABLE IF EXISTS agentconversationturn')
    op.execute('DROP INDEX IF EXISTS ix_agentsession_candidate_name')
    op.execute('DROP TABLE IF EXISTS agentsession')
    # Set defaults for existing NULL values before applying NOT NULL
    op.execute("UPDATE answers SET candidate_answer = '' WHERE candidate_answer IS NULL")
    op.execute("UPDATE answers SET feedback = 'No feedback provided' WHERE feedback IS NULL")
    op.execute("UPDATE answers SET score = 0 WHERE score IS NULL")
    ...
    # Set defaults for interviewsession NULL values
    op.execute("UPDATE interviewsession SET max_questions = 3 WHERE max_questions IS NULL")
    op.execute("UPDATE interviewsession SET current_status = 'INVITED' WHERE current_status IS NULL")
    op.execute("UPDATE interviewsession SET last_activity = now() WHERE last_activity IS NULL")
