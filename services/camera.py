...
class CameraService:
    ...
    def start(self, video_source=0):
        ...
        def init_detectors():
            ...
            # UPDATED PATHS
            known_path = "assets/known_person.jpg"
            gaze_path = "assets/face_landmarker.task"
    ...
    def _process_loop(self):
        ...
        while self.running:
            ...
            now = time.time()
            if self.face_detector and (now - last_face_time) > 0.033:
                last_face_time = now
            ...
            if not self._detectors_ready:
                 gaze_txt = "Loading AI..."
                 warning_msg = "INITIALIZING AI MODELS..."
            elif n_face > 1: 
            ...
            cv2.putText(frame, f"Auth: {found}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, f_clr, 2)
