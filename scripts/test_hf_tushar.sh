...
split_response "$RESP"; [ "$CODE" != "200" ] && echo "Admin Login Failed" && exit 1
ADMIN_TOKEN=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['access_token'])" 2>/dev/null)

TUSHAR_ID=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" "$BASE/admin/users" | python3 -c "import sys,json; d=json.load(sys.stdin); print(next((u['id'] for u in d['data'] if u['email'] == 'tushar@chicmicstudios.in'), ''))" 2>/dev/null)
PAPER_ID=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" "$BASE/admin/papers" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['data'][0]['id'] if d['data'] else '')" 2>/dev/null)
...
split_response "$RESP"; INT_ID=$(echo "$BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['data']['interview']['id'])" 2>/dev/null)
...
split_response "$RESP"; CAND_TOKEN=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['access_token'])" 2>/dev/null)
...
split_response "$RESP"; check "Access Interview" "200" "$CODE" "$BODY"
...
split_response "$RESP"; check "Upload Selfie" "200" "$CODE" "$BODY"
...
Q_ID=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['question_id'])" 2>/dev/null)
...
if [ -n "$Q_ID" ]; then
    RESP=$(curl -s --max-time 30 -w "\n%{http_code}" -X POST "$BASE/interview/submit-answer-text" \
      ...
      -d "interview_id=$INT_ID&question_id=$Q_ID&answer_text=Test+Answer")
